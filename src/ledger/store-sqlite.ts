/**
 * SQLite-backed Store. Single-machine, single-process.
 * `seq` is local insertion order for paging only; `caused_by` is the only causal authority.
 */
...
-- External-proxy claim table. No FK to interaction.hash: claims are acquired
-- before the proposing interaction is appended.
...
export class SqliteStore implements Store {
  ...
  constructor(path: string) {
    ...
    // busy_timeout: make a contending writer wait for the lock instead of erroring.
    this.db.exec('PRAGMA busy_timeout = 5000;')
    // NORMAL is the safe pairing with WAL.
  ...
  async updateLifecycle(
    ...
    // Awaited fanout: live folds re-fold so a superseded/denied interaction leaves the live view.
  ...
  /** Walk parent edges from `of` for `maybeAncestor`. Depth-bounded; returns
   *  false on the limit (treated as concurrent — safer than missing a conflict). */
  ...
  /** Channel frontier = admitted heads with no admitted children in this channel. */
