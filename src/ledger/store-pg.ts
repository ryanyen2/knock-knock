/**
 * Postgres-backed Store — cross-machine path. Same contract as SqliteStore;
 * `subscribe` is driven by `LISTEN interaction_inserted` for cross-machine fanout.
 */
...
-- NOTIFY trigger so other machines see new admissions without polling; payload is the hash.
...
type Row = {
  ...
  created_at: Date
  seq: string // BIGINT returns as string
}
...
export class PgStore implements Store {
  ...
  >()
  /** Local hashes already delivered in `append`; skipped once when their own
   *  NOTIFY echo returns, so a local write isn't delivered twice. */
  ...
  /** Open a pool, ensure the schema, and start LISTENing on `interaction_inserted`. */
  ...
  /** Open (or re-open) the dedicated LISTEN client and re-issue `LISTEN` on every
   *  drop (the registration is session state). Interactions written by other hosts
   *  during a disconnect are missed here — restart the relay to fully re-fold. */
  private async openListener(): Promise<void> {
    ...
    try {
      ...
      // Transient listen failure: the query pool still works; retry in background.
  ...
  async append(i: Interaction): Promise<{ inserted: boolean }> {
    ...
    try {
      ...
      if (inserted) {
        ...
        // Deliver to local subscribers now; remember the hash so the NOTIFY echo is skipped.
        this.locallyDelivered.add(i.hash)
        if (this.locallyDelivered.size > 8192) {
          // Backstop against an unbounded set if the echo never arrives (listener was down).
  ...
  async updateLifecycle(
    ...
    // Awaited in-process fanout. Local-only (NOTIFY fires on INSERT, not UPDATE);
    // cross-machine convergence is re-derived by `apply-supersession` from the
    // winner's immutable `supersedes` op, which crosses via the INSERT NOTIFY.
  ...
  async acquireClaim(
    ...
    try {
      ...
      // No update: someone else's live claim holds.
