...
export type IngestAttachmentDeps = {
  /** Inbound file support + per-file byte cap for `scope`, targeting the addressed
   *  agent's workspace (targetAgent = channel.message args.targetAgent). */
  filesInbound: (scope: string, targetAgent: string | undefined) => { maxBytes: number } | undefined
  ...
  /** Write bytes into the addressed agent's workspace. Undefined when unserved / path escapes. */
  materialize: (scope: string, safeName: string, bytes: Uint8Array, targetAgent: string | undefined) => Promise<string | undefined>
...
export function ingestAttachment(deps: IngestAttachmentDeps): Synchronization {
  ...
  return {
    ...
    fire: async (i, ctx) => {
      const targetAgent =
        i.patch.kind === 'external'
          ? ((i.patch.intent.args as { targetAgent?: unknown } | undefined)?.targetAgent as
              | string
              | undefined)
          : undefined
      const cap = deps.filesInbound(i.channel, targetAgent)
      ...
      for (let idx = 0; idx < list.length; idx++) {
        ...
        const relpath = await deps.materialize(i.channel, safeName, bytes, targetAgent)
        if (!relpath) {
