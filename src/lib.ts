...
// ─── Per-platform onboarding guidance (shared by the wizard and the web UI) ────
// Pure DATA — no functions, no side effects — so the settings server can import it
// (it must never import setup.ts, which runs its wizard on import). setup.ts builds
// its PLATFORMS table by merging this with the id/owner validators above, so the
// "where do I get this" copy lives in exactly one place and can't drift between the
// terminal wizard and the browser surface.
