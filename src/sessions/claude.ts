/**
 * Claude Code session reader. Layout: `<config>/projects/<encoded-cwd>/<id>.jsonl`
 * (`CLAUDE_CONFIG_DIR` or `~/.claude`). The cwd is stored on every line and
 * verified, not trusted from the dir name. Keeps prose + assistant tool_use.
 */
...
// User "messages" that aren't a real prompt; skipped when deriving a title.
...
// A phrase from buildPreamble: marks a relay-driven session (bot talking to itself).
...
/** A human title: Claude's own session summary, else the first real user prompt. */
...
function parseClaude(objs: unknown[]): Parsed {
  ...
  for (const o of objs) {
    ...
    if (type === 'summary' && typeof rec.summary === 'string' && rec.summary) {
      summary = rec.summary // last one wins
      continue
    }
...
/** Candidate project dirs: the exact encoding plus any dir extending it (subdir sessions). */
    ...
    for (const dir of dirs) {
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (isRelayDriven(parsed.events)) continue
          out.push({
          ...
          /* unreadable file — skip, never fail the whole listing */
        }
      }
    }
