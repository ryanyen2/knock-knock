/**
 * Gemini CLI session reader. Layout: `~/.gemini/tmp/<sha256(workspace)>/chats/*.json`
 * — the hash gives an exact project filter, never reads another project's chats.
 */
...
/** Return the message list from a bare Content[] array or a {messages} object. */
...
function parseGemini(raw: unknown, fallbackId: string): Parsed {
  ...
  // Metadata only exists on the object form.
...
export class GeminiSessionStore implements SessionStore {
  ...
  async read(id: string): Promise<NormalizedTranscript | undefined> {
    // No workspace here, so scan every project's chats for a matching id.
    let hashes: string[]
    try {
