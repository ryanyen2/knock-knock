...
import { mergeProposal } from '../../src/ledger/merge.ts'
import {
  LOOP_GUARD_FOLD,
  loopGuardFold,
  stateFor,
  type LoopGuardFoldState,
} from '../../src/ledger/concepts/loop-guard.ts'
import {
  KNOWLEDGE_FOLD,
  knowledgeFold,
  activeNotes,
...
} from '../../src/ledger/artifacts/knowledge.ts'
...
test('cross-machine: bootstrap on a new machine reproduces the existing view', async () => {
  ...
  const { bootstrap } = await import('../../src/ledger/bootstrap.ts')
