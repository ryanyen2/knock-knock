...
import type { VersionableIntent } from '../../../src/ledger/interaction.ts'
...
test('isVersionableEdit: accepts admitted|applied versionable edits, rejects others', async () => {
  const { isVersionableEdit } = await import('../../../src/ledger/artifacts/versionable.ts')
  const base = {
    verb: 'workspace.edit',
...
test('projectVersionable.live lists the kept edits (single edit → itself)', async () => {
  const { liveVersionableEditHashes } = await import('../../../src/ledger/artifacts/versionable.ts')
  const store = new SqliteStore(':memory:')
  const engine = new FoldEngine(store)
  await engine.register(versionableFold)
