 ...
 *
 * Layout under `<data>/storage` (`OPENCODE_DATA_DIR` or
 * `~/.local/share/opencode`):
 *   session/<projectHash>/<sessionID>.json   — meta {id,title,directory,time}
 *   message/<sessionID>/<messageID>.json     — {id,role,parts?}
 *   part/.../<partID>.json                    — {type:'text',text} | {type:'tool',…}
 * Parts may be inlined on the message or split into part files; schemas have
 * shifted across versions, so we probe both and tolerate misses. Sessions are
 * filtered by the session meta's `directory`.
 */
