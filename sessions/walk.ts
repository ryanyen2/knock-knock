/**
 * Shared recursive directory walk for the session readers. The codex reader
 * (YYYY/MM/DD nesting) and the opencode reader (split storage dirs) both need
 * "collect every *.<ext> under a root, depth-bounded, tolerant of unreadable
 * dirs" — one implementation here instead of one per reader.
 */
