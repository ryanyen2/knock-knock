...
export class FoldEngine {
  ...
  private readonly subscribers = new Map<
    string,
    Set<(state: unknown, delta: Interaction) => void>
  >()
  ...
  subscribe<S>(name: string, cb: (state: S, delta: Interaction | undefined) => void): () => void {
    ...
    const wrapped = cb as (state: unknown, delta: Interaction) => void
    set.add(wrapped)
  ...
  /** Tear down the store subscription. Tests call this; production lets it persist. */
  close(): void {
  ...
  private applyTo(entry: FoldEntry, i: Interaction): void {
