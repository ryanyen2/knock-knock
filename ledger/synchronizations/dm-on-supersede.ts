/**
 * dm-on-supersede — §4.4 "your draft was overridden" inbox note → owner DM.
 *
 * The merge gate (admit.ts) already writes a `knowledge.append` to the loser's
 * inbox (`know:actor/<actor>/inbox`, authored by `system:merge-gate`) whenever
 * a proposal is superseded or rejected lower-role. That note IS the surface-
 * back mechanism for the agent's next turn. §4.4 adds the human side: DM the
 * losing agent's owner so they know their bot's draft was overridden.
 *
 * Pure ledger logic; the owner lookup and the actual DM send are injected so
 * this stays testable without Discord (rubric #2: one new file, zero edits to
 * concepts).
 */
...
export function dmOnSupersede(opts: DmOnSupersedeOpts): Synchronization {
  return {
    ...
    fire: async i => {
      ...
      if (!ownerId) return // not an agent we host, or no owner — nothing to DM
